...
async function runEnrichmentV2() {
  console.log("🚀 Starting database-wide enrichment V2 (Plain English summaries, use cases, and step-by-step guides) across all 11,468 models...\n");
  ...
  console.log(`\n✅ Loaded ${allModels.length} models. Processing plain-English summaries, real-world use cases, and step-by-step setup guides...\n`);

  let enrichedCount = 0;

  // 2. Apply enrichModelData
  const enrichedModels: AIModel[] = allModels.map((model) => {
    enrichedCount++;
    const enriched = enrichModelData(model);

    // Clean up null / created_at for strict JSON typing
    ...
    Object.keys(clean).forEach((k) => {
      if (clean[k] === null) {
        if (k === "featured" || k === "trending" || k === "isDeprecated" || k === "hasApi" || k === "hasSelfHost") {
          clean[k] = false;
        } else if (k === "useCases" || k === "modalities" || k === "benchmarks" || k === "strengths" || k === "weaknesses" || k === "tags" || k === "aliases" || k === "realWorldExamples" || k === "howToUseSteps") {
          clean[k] = [];
        } else if (k === "hardwareRequirements" || k === "bestForCategory" || k === "plainEnglishSummary") {
          delete clean[k];
        } else {
          clean[k] = "";
        }
  ...
  // Prepare Supabase records (strip UI-only fields not in SQL table schema)
  const supabaseRecords = enrichedModels.map((m) => {
    const rec: any = { ...m };
    delete rec.plainEnglishSummary;
    delete rec.realWorldExamples;
    delete rec.howToUseSteps;
    return rec;
  });

  console.log(`🔄 Applying V2 enriched data back to Supabase Postgres in batches of 200...`);
  ...
  for (let i = 0; i < supabaseRecords.length; i += BATCH_SIZE) {
    ...
    if (error) {
      ...
      if (successCount % 1000 === 0 || successCount === supabaseRecords.length) {
        console.log(`✅ Progress: ${successCount} / ${supabaseRecords.length} models enriched V2 in Supabase...`);
      }
    }
  }
  ...
  console.log(`💾 Saved ${enrichedModels.length} V2 enriched models to src/data/models_db.json.`);
  ...
  console.log(`💾 Saved top 1,000 V2 enriched models to src/data/models.ts.`);

  console.log("\n================ ENRICHMENT V2 SUMMARY ================");
  console.log(`Total models processed:         ${enrichedCount}`);
  console.log(`Successfully updated Supabase: ${successCount}`);
  console.log(`Failed records:                ${failCount}`);
  console.log("========================================================\n");
}

runEnrichmentV2().catch((err) => {
  console.error("Fatal error during V2 enrichment:", err);
  process.exit(1);
});
